...
const poll = {
  ...
  registerNewAnswer(arrOrStr, fn) {
    const pollQ = Number(
      // Getting the answer from the user
      prompt(
        `${this.question}\n${this.options}\n(Write your option below)`.replaceAll(
          `,`,
          `\n`
        )
      )
    );
    // Register answer to the array
    if (typeof pollQ === `number` && pollQ < this.answers.length) {
      poll.answers[pollQ]++;
    } else {
      console.log(`Please enter a number between 0-3`);
    }
